...
contextBridge.exposeInMainWorld('carbonsync', {
  ...
  addFolder: (path, name, direction, folderId) => ipcRenderer.invoke('add-folder', path, name, direction, folderId),
  ...
  cleanBackups: () => ipcRenderer.invoke('clean-backups'),
  syncDiag: () => ipcRenderer.invoke('sync-diag'),
  forcePush: (folder) => ipcRenderer.invoke('force-push', folder),
  ...
  onGameLibraryUpdated: (cb) => { ipcRenderer.on('game-library-updated', (_, d) => { try { cb(d); } catch {} }); },
